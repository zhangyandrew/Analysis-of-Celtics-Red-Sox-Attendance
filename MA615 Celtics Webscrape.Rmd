...
url2012_len <- length(match_g1)
urlelse_len <- length(match_g2)

## 2012 Season
for (i in 1:url2012_len+1){
...
```{r per 1 game example}
html <- paste(readLines(urls[1]), collapse="\n")
matched <- str_match_all(html, "<a href=\"(.*?)\"")
matched <- data.frame(matched)

match_g <- matched$X2[sapply("http://www.espn.com/nba/game", grepl, matched$X2)]
match_g <- as.character(match_g)

url2012_len <- length(match_g)
#url20_len <- length(match_else)

attend <- c()

for (i in 1:url2012_len){
  attend[i] <- readLines(match_g[i])[grep("text/javascript", readLines(match_g[i]))]
}

for (i in 1:url2012_len){
  attend[i] <- readLines(match_g[i])[grep("Capacity", readLines(match_g[i]))]
  attend[i] <- rm_between(readLines(match_g[i])[grep("Capacity", readLines(match_g[i]))], "<div class=\"game-info-note capacity\">Capacity:", "</div>", extract = TRUE)
}

attend <- data.frame(attend, stringsAsFactors = FALSE)
colnames(attend) <- "Attendance"
#attend$Attendance <- as.numeric()
```

```{r combine}
attend2012 <- data.frame(t(attend1))
attend2012 <- cbind(attend2012, cdata1$NULL.V1, cdata1$NULL.V2, hwdata1)

attend2013 <- data.frame(t(attend2))
attend2013 <- cbind(attend2013, cdata2$NULL.V1[-81], cdata2$NULL.V2[-81], hwdata2[-81,])
...
attend2015 <- cbind(attend2015, cdata4$NULL.V1, cdata4$NULL.V2, hwdata4)

attend2016 <- data.frame(t(attend5))
attend2016 <- cbind(attend2016,  cdata5$NULL.V1[-47], cdata5$NULL.V2[-47], hwdata5[-47])
...
attend2012$Date[1] <- str_c(attend2012$Date[1], " 2011")
attend2012$Date[2:33] <- str_c(attend2012$Date[2:33], " 2012")

attend2013$Date[1:14] <- str_c(attend2013$Date[1:14], " 2012")
attend2013$Date[15:40] <- str_c(attend2013$Date[15:40], " 2013")

attend2014$Date[1:17] <- str_c(attend2014$Date[1:17], " 2013")
attend2014$Date[18:41] <- str_c(attend2014$Date[18:41], " 2014")

attend2015$Date[1:17] <- str_c(attend2015$Date[1:17], " 2014")
attend2015$Date[18:41] <- str_c(attend2015$Date[18:41], " 2015")

attend2016$Date[1:17] <- str_c(attend2016$Date[1:17], " 2015")
attend2016$Date[18:42] <- str_c(attend2016$Date[18:42], " 2016")
